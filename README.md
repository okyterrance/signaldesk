...
story with a transparent seven-factor score, and pushes a written
...
factor            raw     wt   adds
───────────────────────────────────
keyword          1.00  0.222  0.222
source_quality   0.95  0.220  0.209
recency          0.94  0.222  0.208
category         1.00  0.145  0.145
numeric          1.00  0.056  0.056
asset            0.50  0.056  0.028
...
12 RSS feeds (concurrent, independently timed out)
 ...
 Stage 1 · dedupe                TF-IDF cosine ∪ proper-noun overlap,
        ...
        ▼
 Stage 2 · 7-factor score        weighted sum, normalised to 0–1,
        │                        every score carries its breakdown
        ▼
 Stage 3 · adaptive top-N        quality gate first, fixed count only
        │                        as a floor — a quiet day gets a short
        │                        digest, not padding
        ▼
...
|---|---|---|
| `keyword` | 0.222 | 4 tiers, highest hit wins. `fed`/`cpi`/`hack` → 1.0, `btc`/`defi` → 0.25 |
| `recency` | 0.222 | linear decay to zero across 24h |
| `source_quality` | 0.220 | per-outlet, FT/Bloomberg/The Block 1.0 → unrated 0.60 |
| `category` | 0.145 | topical gate from feed tags; untagged scores neutral, not zero |
| `source_count` | 0.080 | corroboration across outlets, saturating at 5 |
| `numeric` | 0.056 | headline carries a concrete figure |
| `asset` | 0.056 | BTC/ETH 1.0, large alts 0.7, memecoins 0.3 |
...
```bash
python -m pytest tests/ -q      # 59 tests, no network, no API key
```
